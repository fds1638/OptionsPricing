...
class HeathJarrowMorton():
    ...
    def run_ho_lee_simulations(self, number_of_runs: int, ho_lee: ho_lee, f0: Callable):
        """Given Ho-Lee and forward function f0 = f(0,T), return interest rates and size of Money savings account."""
        run_results_retval = []
        M_matrix_retval = []
    ...
    def average_ho_lee_simulations(self, number_of_runs, time_values, simulation_run_results, simulation_M_matrix):
        """Average Ho-Lee results for interest rate and for Money savings account growth."""
    ...
    def graph_exponential_example(self) -> None:
        if self.interest_rate_curve == "exponential":
            ...
            averaged_results, P_graph = self.average_ho_lee_simulations(self.number_of_runs, time_values, run_results, M_matrix)
